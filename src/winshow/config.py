...
#: List-valued settings are annotated with `NoDecode` so that pydantic-settings hands the
#: raw environment string to the validator below instead of trying to JSON-decode it
#: first. Without it, `WINSHOW_AGENT_TOKENS=a,b` fails at startup with a JSONDecodeError
#: — the documented comma-separated form would never have worked, and only an
#: environment-driven test catches it, because constructing Settings(...) in Python
#: bypasses the environment source entirely.
...
class Settings(BaseSettings):
    ...
    agent_tokens: CsvList = Field(default_factory=list)
    ...
    #: extension point.
    allowed_agent_ids: CsvList = Field(default_factory=list)
